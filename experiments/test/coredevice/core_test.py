...
import demo_system.coredevice.core


class _Core(demo_system.coredevice.core.Core):
    ...
    def compile(self, *args, **kwargs):
        embedding_map, kernel_library, symbolizer, demangler = super(_Core, self).compile(*args, **kwargs)
...
class CoreTestCase(unittest.TestCase):
    ...
    def test_string_to_bytes(self):
        ...
        for s, r in data:
            self.assertEqual(demo_system.coredevice.core._str_to_bytes(s), r)
...
class CoreCompilingTestCase(unittest.TestCase):
    ...
    # Todo: test cases are failing in Kernel. I believe they have to be run on connected to a Kasli
    ...
    #     for size in sizes:
    #         self._set_core(max_kernel_size=f'{size * 4 + ref_size}b')
    #         self.data = np.empty(size, dtype=np.int32)
    #         self._update_kernel_invariants('data')
    #         self._kernel()  # Fits exactly, does not raise
    #         with self.assertRaises(demo_system.coredevice.core.KernelSizeException,
    #                                msg=f'Kernel size exception did not raise for size {size}'):
    ...
    #     self.data = np.empty(1, dtype=np.int32)
    #     self._update_kernel_invariants('data')
    #     with self.assertLogs(demo_system.coredevice.core._logger, logging.DEBUG):
    #         self._kernel()
